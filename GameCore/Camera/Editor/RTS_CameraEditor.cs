...
namespace RTS_Cam
{
    ...
    {
        private CameraController camera { get { return target as CameraController; } }
        ...
        //script  = EditorGUILayout.ObjectField(camera, typeof(MonoScript), true) as MonoScript;
            //base.OnInspectorGUI();
         ...
         /*   EditorGUILayout.EndVertical();
            ...
            r = new Rect(r.x - 13, r.y - 17, EditorGUIUtility.currentViewWidth, r.height + 20);
            EditorGUI.DrawRect(r, new Color(0.0f, 0.0f, 1f, 0.3f));*/
        ...
        private void MovementTab()
        {
            ...
            if(camera.useKeyboardInput)
            {
                ...
                camera.speedUP = EditorGUILayout.FloatField("Speed UP: ", camera.speedUP);
            }
            ...
            using (new HorizontalBlock())
            {
                GUILayout.Label("Automatic Target Follow : ", EditorStyles.boldLabel, GUILayout.Width(170f));
                camera.followTarget = EditorGUILayout.Toggle(camera.followTarget);
            }
            if (camera.followTarget)
            {
                camera.followingSpeed = EditorGUILayout.FloatField("Following speed: ", camera.followingSpeed); // Automatický follow - speed
                EditorGUILayout.Space();

            }
            // Obecné pole camera follow
            camera.targetFollow = EditorGUILayout.ObjectField("Target to follow: ", camera.targetFollow, typeof(Transform)) as Transform;
            camera.distanceFromTarget = EditorGUILayout.FloatField("Min distance from target to follow: ", camera.distanceFromTarget);
            camera.targetOffset = EditorGUILayout.Vector3Field("Target offset: ", camera.targetOffset);

            using (new HorizontalBlock())
            {
                GUILayout.Label("Zoom on target: ", EditorStyles.boldLabel, GUILayout.Width(170f));
            }
                camera.targetZoomSpeed = EditorGUILayout.FloatField("Zoom speed", camera.targetZoomSpeed);
        ...
        private void RotationTab()
        {
            ...
            {
                camera.invertY = EditorGUILayout.Toggle("Invert mouse Y?", camera.invertY);

                GUILayout.Label("Mouse rotation sensitivity: ", EditorStyles.boldLabel, GUILayout.Width(170f));
                camera.mouseMovementSensitivity = EditorGUILayout.Slider(camera.mouseMovementSensitivity, 0.1f, 1f);
            }
